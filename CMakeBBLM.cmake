...
#
# Copyright (c) 2016 Sascha Kratky
#
...
function(resolve_templates _propNames)
	...
	foreach (_unresolvedPropName ${ARGN})
		if (_unresolvedPropName MATCHES "<CONFIG>")
			foreach (_config "None" "Debug" "MinSizeRel" "Release" "RelWithDebInfo")
		...
		elseif (_unresolvedPropName MATCHES "\\(([0-9]+)..([0-9]+)\\)")
			...
			foreach (_index RANGE ${CMAKE_MATCH_1} ${CMAKE_MATCH_2})
				string (REPLACE "(0..9)" "${_index}" _propName "${_unresolvedPropName}")
				list (APPEND ${_propNames} ${_propName})
			endforeach()
		elseif (_unresolvedPropName MATCHES "<[a-zA-Z0-9_-]+>")
			message(STATUS "Skipping ${_unresolvedPropName}")
		else()
			list (APPEND ${_propNames} ${_unresolvedPropName})
		endif()
	endforeach()
	list (REMOVE_DUPLICATES ${_propNames})
